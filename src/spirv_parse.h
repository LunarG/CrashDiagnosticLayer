...
class SpirvParse {
   public:
    ...
    Result Parse(size_t size_in_bytes, const char* p_bytes) {
        // Word count check
        const uint32_t spirv_word_count = static_cast<uint32_t>(size_in_bytes) / SpirvParse::kSpirvWordSize;
        ...
        while (current_spirv_word_index_ < spirv_word_count) {
            ...
            SpirvParse::Result op_result = ParseOperands(op, operand_word_count, p_operand_words);
   ...
   protected:
    ...
    virtual SpirvParse::Result ParseOperands(spv::Op op, uint32_t operand_word_count, const uint32_t* p_words) = 0;
    virtual void ParseCompleteEvent() {}

    template <typename T>
    SpirvParse::Result ReadOperandWord(uint32_t word_index, uint32_t word_count, const uint32_t* p_words,
                                       T* p_out_word) const {
    ...
    SpirvParse::Result ReadString(uint32_t word_index, uint32_t word_count, const uint32_t* p_words,
...
class BasicSpirvParse : public SpirvParse {
   ...
   protected:
    virtual SpirvParse::Result ParseOperands(spv::Op op, uint32_t operand_word_count, const uint32_t* p_words) {
        // printf("OP %08x\n", (uint32_t)op);
        switch (op) {
            default:
                break;
            ...
            case spv::OpString: {
                ...
                SpirvParse::Result result = ReadOperandWord<uint32_t>(0, operand_word_count, p_words, &result_id);
            ...
            case spv::OpSource: {
                ...
                // Source Language
                SpirvParse::Result result =
                    ReadOperandWord<spv::SourceLanguage>(0, operand_word_count, p_words, &source_language_);
                if (result != SpirvParse::Result::kSuccess) {
                    return result;
                }
                // Version
                result = ReadOperandWord<uint32_t>(1, operand_word_count, p_words, &version_);
                if (result != SpirvParse::Result::kSuccess) {
                    return result;
                }
                // File
                if (operand_word_count > 2) {
                    result = ReadOperandWord<uint32_t>(2, operand_word_count, p_words, &file_id_);
                    if (result != SpirvParse::Result::kSuccess) {
                        return result;
                    }
                    // Find the file name
            ...
            case spv::OpEntryPoint: {
                ...
                // Execution Model
                SpirvParse::Result result =
                    ReadOperandWord<spv::ExecutionModel>(0, operand_word_count, p_words, &execution_model_);
                if (result != SpirvParse::Result::kSuccess) {
                    return result;
                }
   ...
   private:
    spv::SourceLanguage source_language_ = spv::SourceLanguage::SourceLanguageUnknown;
    uint32_t version_ = 0;
