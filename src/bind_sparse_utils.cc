...
bool BindSparseUtils::BindSparseInfoWaitsOnBinarySemaphores(const VkBindSparseInfo* bind_info,
    ...
    for (uint32_t i = 0; i < bind_info->waitSemaphoreCount; i++) {
        if (semaphore_tracker->GetSemaphoreType(bind_info->pWaitSemaphores[i]) == VK_SEMAPHORE_TYPE_BINARY_KHR) {
            return true;
        }
    }
    return false;
...
bool BindSparseUtils::ShouldExpandQueueBindSparseToTrackSemaphores(
    ...
    for (uint32_t i = 0; i < packed_bind_sparse_info->bind_info_count; i++) {
        if (BindSparseInfoWaitsOnBinarySemaphores(&packed_bind_sparse_info->bind_infos[i],
...
void BindSparseUtils::GetWaitBinarySemaphores(const VkBindSparseInfo* bind_info,
                                              const SemaphoreTracker* semaphore_tracker,
...
// accordingly.
void BindSparseUtils::ExpandBindSparseInfo(ExpandedBindSparseInfo* bind_sparse_expand_info) {
    ...
    for (uint32_t i = 0; i < packed_bind_sparse_info->bind_info_count; i++) {
        ...
        if (BindSparseInfoWaitsOnBinarySemaphores(&packed_bind_sparse_info->bind_infos[i],
            ...
            if (timeline_semaphore_info) {
                ...
                bind_sparse_expand_info->timeline_semaphore_infos.push_back(vk_timeline_semaphore_submit_info);
                auto& tsinfo = bind_sparse_expand_info->timeline_semaphore_infos.back();
                tsinfo.waitSemaphoreValueCount = timeline_semaphore_info->waitSemaphoreValueCount;
                tsinfo.pWaitSemaphoreValues = timeline_semaphore_info->pWaitSemaphoreValues;
            } else {
                bind_sparse_expand_info->has_timeline_semaphore_info.push_back(false);
            }
            submit_info.waitSemaphoreCount = packed_bind_sparse_info->bind_infos[i].waitSemaphoreCount;
            submit_info.pWaitSemaphores = packed_bind_sparse_info->bind_infos[i].pWaitSemaphores;
            ...
            bind_sparse_expand_info->wait_binary_semaphores.push_back(std::vector<VkSemaphore>());
            auto& wbsemaphores = bind_sparse_expand_info->wait_binary_semaphores.back();
            GetWaitBinarySemaphores(&packed_bind_sparse_info->bind_infos[i], packed_bind_sparse_info->semaphore_tracker,
                                    &wbsemaphores);
        }
        ...
        if (packed_bind_sparse_info->bind_infos[i].signalSemaphoreCount > 0) {
            ...
            if (timeline_semaphore_info) {
                ...
                bind_sparse_expand_info->timeline_semaphore_infos.push_back(vk_timeline_semaphore_submit_info);
                auto& tsinfo = bind_sparse_expand_info->timeline_semaphore_infos.back();
                tsinfo.signalSemaphoreValueCount = timeline_semaphore_info->signalSemaphoreValueCount;
                tsinfo.pSignalSemaphoreValues = timeline_semaphore_info->pSignalSemaphoreValues;
            } else {
                bind_sparse_expand_info->has_timeline_semaphore_info.push_back(false);
            }
            submit_info.signalSemaphoreCount = packed_bind_sparse_info->bind_infos[i].signalSemaphoreCount;
            submit_info.pSignalSemaphores = packed_bind_sparse_info->bind_infos[i].pSignalSemaphores;
        }
    }
}
...
std::string BindSparseUtils::LogBindSparseInfosSemaphores(const Device& device, VkDevice vk_device, VkQueue vk_queue,
    ...
    for (uint32_t i = 0; i < bind_info_count; i++) {
        if (bind_infos[i].waitSemaphoreCount == 0 && bind_infos[i].signalSemaphoreCount == 0) {
            continue;
        }
        ...
        if (timeline_semaphore_info) {
            for (uint32_t j = 0; j < timeline_semaphore_info->waitSemaphoreValueCount; j++) {
                if (semaphore_tracker->GetSemaphoreType(wait_semaphores[j]) == VK_SEMAPHORE_TYPE_TIMELINE_KHR) {
                    wait_semaphore_values[j] = timeline_semaphore_info->pWaitSemaphoreValues[j];
                }
            }
            for (uint32_t j = 0; j < timeline_semaphore_info->signalSemaphoreValueCount; j++) {
                if (semaphore_tracker->GetSemaphoreType(signal_semaphores[j]) == VK_SEMAPHORE_TYPE_TIMELINE_KHR) {
                    signal_semaphore_values[j] = timeline_semaphore_info->pSignalSemaphoreValues[j];
                }
            }
        }
