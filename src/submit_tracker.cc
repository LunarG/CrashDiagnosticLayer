...
SubmitTracker::SubmitTracker(Device& device) : device_(device) {}
...
void SubmitTracker::StoreSubmitHelperCommandBuffersInfo(SubmitInfoId submit_info_id, VkCommandPool vk_pool,
...
void SubmitTracker::RecordSubmitStart(QueueSubmitId qsubmit_id, SubmitInfoId submit_info_id,
    ...
    if (submit_infos_.find(submit_info_id) != submit_infos_.end()) {
        ...
        for (size_t i = 0; i < submit_info.wait_semaphores.size(); i++) {
            if (semaphore_tracker->GetSemaphoreType(submit_info.wait_semaphores[i]) == VK_SEMAPHORE_TYPE_BINARY_KHR) {
                semaphore_tracker->WriteMarker(submit_info.wait_semaphores[i], vk_command_buffer,
...
void SubmitTracker::RecordSubmitFinish(QueueSubmitId qsubmit_id, SubmitInfoId submit_info_id,
                                       VkCommandBuffer vk_command_buffer) {
    assert(submit_info_id != kInvalidSubmitInfoId);
    std::lock_guard<std::mutex> slock(submit_infos_mutex_);
    if (submit_infos_.find(submit_info_id) != submit_infos_.end()) {
        ...
        for (size_t i = 0; i < submit_info.signal_semaphores.size(); i++) {
            semaphore_tracker->WriteMarker(submit_info.signal_semaphores[i], vk_command_buffer,
                                           VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, submit_info.signal_semaphore_values[i],
                                           {SemaphoreModifierType::kModifierQueueSubmit, qsubmit_id});
        }
        // Write the state of the submit
    ...
    for (auto qsubmit_it = queue_submits_.begin(); qsubmit_it != queue_submits_.end();) {
        ...
        for (auto submit_it = queue_submit_info_ids.begin(); submit_it != queue_submit_info_ids.end();) {
            ...
            if (submit_status == SubmitState::kFinished) {
                ...
                device_.FreeCommandBuffers(submit_info.helper_cbs_command_pool, 2, tracking_buffers.data());
...
void SubmitTracker::RecordBindSparseHelperSubmit(QueueBindSparseId qbind_sparse_id, const VkSubmitInfo* vk_submit_info,
...
void SubmitTracker::CleanupBindSparseHelperSubmits() {
    ...
    for (auto helper_submit_info = helper_submit_infos_.begin(); helper_submit_info != helper_submit_infos_.end();) {
        ...
        if (submit_status == SubmitState::kFinished) {
            ...
            // the values of the semaphores
            device_.FreeCommandBuffers(helper_submit_info->command_pool, 1, &(helper_submit_info->marker_cb));
...
bool SubmitTracker::QueuedSubmitWaitingOnSemaphores(SubmitInfoId submit_info_id) const {
    auto it = submit_infos_.find(submit_info_id);
    const SubmitInfo& submit_info = it->second;
...
std::vector<TrackedSemaphoreInfo> SubmitTracker::GetTrackedSemaphoreInfos(SubmitInfoId submit_info_id,
...
std::string SubmitTracker::GetSubmitInfoSemaphoresLog(VkDevice vk_device, VkQueue vk_queue,
