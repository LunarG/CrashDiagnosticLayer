...
TEST_F(Settings, WatchdogTimeout) {
    ...
    layer_settings_.message_severity = {"info"};
...
TEST_F(Settings, LogFilePath) {
    ...
    layer_settings_.log_file = kLogFileName;
    ...
    std::filesystem::directory_iterator output_iter(this->output_path_);
    std::filesystem::directory_iterator end;

    ASSERT_FALSE(output_iter == end);
    ASSERT_TRUE(output_iter->is_directory());

    auto rel = std::filesystem::relative(output_iter->path(), this->output_path_).string();
...
TEST_F(Settings, LogFileLeadingDot) {
    ...
    layer_settings_.log_file = s.c_str();
    // make sure something gets logged.
    layer_settings_.message_severity = {"error", "warn", "info", "verbose"};
    InitInstance();
